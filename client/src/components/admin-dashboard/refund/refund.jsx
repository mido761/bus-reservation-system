...
export default function refund() {
  ...
  // Get unique statuses for dropdowns
  const getUniqueStatuses = (arr, key) => {
    return Array.from(new Set(arr.map((item) => item[key]).filter(Boolean)));
  };

  const paymentStatusOptions = useMemo(
    () => getUniqueStatuses(results, "payment_status"),
    [results]
  );

  // Filter results by dropdowns
  const filteredResults = useMemo(() => {
    return results.filter((row) => {
      const matchesPayment = !paymentStatus || row.payment_status === paymentStatus;
          ...
          <div className="flex flex-col gap-4 items-center w-full">
            ...
            <div className="w-full flex flex-col md:flex-row gap-4 items-center">
              <select
                ...
                className="border rounded-lg py-2 px-3 focus:ring-2 focus:ring-blue-500 w-full md:w-40"
              >
